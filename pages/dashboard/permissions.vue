  ...
  <div class="page-container">
    <div class="page-header">
      <div class="page-header__title">
        <h1>الصلاحيات</h1>
        <p>إدارة صلاحيات النظام والمستأجرين</p>
      </div>
    ...
    <div class="card" style="margin-bottom:1.5rem;padding:1rem 1.5rem">
      <div class="filters-row">
        <div class="search-bar" style="flex:1;min-width:200px">
          ...
          <input v-model="search" type="text" placeholder="بحث في الصلاحيات..." />
        </div>
      ...
      <table v-else class="data-table">
        ...
        <tbody>
          <tr v-for="perm in filtered" :key="perm.id">
            ...
            <td><span :class="`badge badge--${perm.scope}`">{{ perm.scope === 'system' ? 'نظام' : 'مستأجر' }}</span></td>
            <td>
              <span v-if="perm.tenantId" class="text-xs text-muted">{{ perm.tenantId.slice(0, 8) }}…</span>
            ...
            <td>
              <div style="display:flex;gap:0.5rem">
                ...
                >حذف</button>
                <span v-if="perm.scope === 'system'" class="text-xs text-muted">للقراءة فقط</span>
              </div>
            </td>
          </tr>
        </tbody>
          ...
          <div class="modal">
            ...
            <form @submit.prevent="handleSubmit">
              <div class="form-group">
                ...
                <input v-model="form.name" type="text" class="form-input" placeholder="create_invoice" required />
                <span class="form-error" v-if="!form.name && submitted">هذا الحقل مطلوب</span>
              </div>
...
const filtered = computed(() =>
  store.permissions.filter(p => {
    ...
    return (!q || p.name.toLowerCase().includes(q)) && (!scopeFilter.value || p.scope === scopeFilter.value)
  })
)

const openCreate = () => { editing.value = null; form.name = ''; showModal.value = true }
...
const confirmDelete = (p: Permission) => { deleteTarget.value = p; showConfirm.value = true }
