    ...
    <div class="card" style="margin-bottom: 1.5rem; padding: 1rem 1.5rem">
      <div class="filters-row">
        <div class="search-bar" style="flex:1;min-width:200px">
          ...
          <input v-model="search" type="text" placeholder="بحث باسم المستخدم أو البريد..." />
        </div>
              ...
              <td>
                <div style="display:flex;align-items:center;gap:0.5rem">
                  ...
                  <div>
                    ...
                    <div style="font-size:0.75rem;color:#4a6a8a">{{ formatDate(user.created_at) }}</div>
                  </div>
                </div>
              </td>
              <td>{{ user.email }}</td>
              <td><span :class="`badge badge--${user.status}`">{{ statusLabel(user.status) }}</span></td>
              <td>
                <span v-if="user.role">{{ user.role.name }}</span>
                <span v-else class="text-muted text-xs">بدون دور</span>
              </td>
              <td>
                <span v-if="user.isSuperAdmin" class="badge badge--system">مدير المستأجر</span>
                <span v-else-if="user.isSystemAdmin" class="badge badge--active">مدير النظام</span>
                <span v-else class="badge badge--tenant">مستخدم</span>
              </td>
              <td>
                <div style="display:flex;gap:0.5rem">
                  <button class="btn btn--ghost btn--sm" @click="openEdit(user)">تعديل</button>
                  <button class="btn btn--danger btn--sm" @click="confirmDelete(user)">حذف</button>
                </div>
              </td>
            </tr>
          </tbody>
            ...
            <form @submit.prevent="handleSubmit">
              <div style="display:flex;flex-direction:column;gap:1rem">
                <div class="form-group">
                  ...
                  <input v-model="form.username" type="text" class="form-input" placeholder="employee_john" required />
                </div>
                <div class="form-group">
                  <label>البريد الإلكتروني *</label>
                  <input v-model="form.email" type="email" class="form-input" placeholder="john@company.com" :required="!editingUser" />
                </div>
                <div v-if="!editingUser" class="form-group">
                  <label>كلمة المرور *</label>
                  <input v-model="form.password" type="password" class="form-input" placeholder="••••••••" minlength="6" required />
                </div>
                <div class="form-group">
                  <label>الدور</label>
              ...
              <div class="modal__footer">
                ...
                <button type="submit" class="btn btn--primary" :disabled="submitting">
                  ...
                  <span v-else>{{ editingUser ? 'حفظ التغييرات' : 'إنشاء المستخدم' }}</span>
                </button>
              </div>
            </form>
          </div>
...
const openCreate = () => {
  ...
  Object.assign(form, { username: '', email: '', password: '', roleId: '', status: 'active' })
...
const statusLabel = (s: string) => ({ active: 'نشط', inactive: 'غير نشط', suspended: 'موقوف' }[s] ?? s)
